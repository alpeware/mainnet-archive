...
const receipts = async (provider, transactions, process) =>
    Promise.all(transactions.map((e) => provider.getTransactionReceipt(e.hash).then((f) => Object.assign(e, f)).then((g) => process(g))))

const fetch = async (provider, transactions, process) => {
  return receipts(provider, transactions, process)
}
